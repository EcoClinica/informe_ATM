...
st.markdown("""
    ...
    .resultado-calculo { background-color: #E0F2FE; padding: 10px; border-radius: 5px; border-left: 4px solid #0284C7; margin-top: 10px; font-size: 14px; }
...
def componente_microfono(key_prefijo):
    ...
    function iniciarDictado(prefix) {{
        ...
        recognition.onresult = function(event) {{
            ...
            // Buscar todos los números enteros o decimales en la frase dictada
            const matches = texto.replace(/,/g, '.').match(/[0-9]+(\.[0-9]+)?/g);
            
            if (matches && matches.length >= 3) {{
                // Enviar los datos directamente a las cajas de texto de la aplicación principal
...
def calcular_posicion_condilo(ant_sup_txt, post_inf_txt):
    try:
        ...
        if (pi_val + as_val) == 0: return "Esperando medidas..."
...
with col_der:
    ...
    abierta_txt_der = st.selectbox("Boca abierta (D):", opts_boca_abierta, key="a_txt_der")
...
with col_izq:
    ...
    abierta_txt_izq = st.selectbox("Boca abierta (I):", opts_boca_abierta, key="a_txt_izq")
...
# --- SECCIÓN DE DESCARGA CORREGIDA PARA EL IPAD ---
st.subheader("💾 Finalizar Informe")

try:
    # Preparamos los datos del Word en memoria continuamente para que esté listo al pulsar
    ...
    contexto = {
        ...
        'hora_cerrada_der': hora_cerrada_der, 'cerrada_txt_der': cerrada_txt_der, 'abierta_txt_der': abierta_txt_der,
        ...
        'hora_cerrada_izq': hora_cerrada_izq, 'cerrada_txt_izq': cerrada_txt_izq, 'abierta_txt_izq': abierta_txt_izq,
    ...
    # Guardar el archivo en un búfer binario en memoria
    bio = io.BytesIO()
    doc.save(bio)
    bio.seek(0)
    ...
    # Botón nativo de descarga para que funcione en iOS/iPad y navegadores web
    st.download_button(
    ...
    st.error("Asegúrate de que el archivo 'plantilla_atm.docx' está guardado en la misma carpeta que este programa.")